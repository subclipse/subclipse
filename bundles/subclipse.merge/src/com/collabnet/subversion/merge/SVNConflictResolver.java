...
public class SVNConflictResolver implements ISVNConflictResolver {
  ...
  private ConflictResolution getConflictResolution(final SVNConflictDescriptor descrip) {
              ...
              new Runnable() {
                public void run() {
                  ...
                  MergeWizardDialog dialog =
                      new MergeWizardDialog(Display.getDefault().getActiveShell(), dialogWizard);
                  dialog.open();
                }
              });
    ...
    try {
      ...
      if (conflictResolution.getResolution() == ConflictResolution.CONFLICT_EDITOR) {
        ...
        final MergeEditConflictsAction editConflictsAction =
        ...
        while (editConflictsAction.getMergeConflictsCompareInput() == null
            || !editConflictsAction.getMergeConflictsCompareInput().isFinished()) {
          try {
            Thread.sleep(1000);
          } catch (InterruptedException e) {
          }
        ...
        if (editConflictsAction.getMergeConflictsCompareInput().isResolved()) {
          ...
          conflictResolution =
              new ConflictResolution(
                  descrip, editConflictsAction.getMergeConflictsCompareInput().getResolution());
        } else {
          //					conflictResolution = new ConflictResolution(descrip,
              ...
              new Runnable() {
                public void run() {
                  ...
                  MessageDialog.openError(
                      ...
                      Messages.SVNConflictResolver_handleConflict,
                      Messages.SVNConflictResolver_conflictNotHandled
                          + re.getLocalizedMessage()
                          + Messages.SVNConflictResolver_resolveLater);
                  re.printStackTrace();
                  wait = false;
                }
              });
  ...
  public int getResolvedConflictCount(int conflictKind) {
    if (textHandling == ISVNConflictResolver.Choice.postpone
        && binaryHandling == ISVNConflictResolver.Choice.postpone
        && treeConflictHandling == ISVNConflictResolver.Choice.postpone) return 0;
