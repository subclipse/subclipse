...
//					conflictResolution = new ConflictResolution(descrip, ISVNConflictResolver.Result.resolved);
					conflictResolution = new ConflictResolution(descrip, editConflictsAction.getMergeConflictsCompareInput().getResolution());
				} else {
//					conflictResolution = new ConflictResolution(descrip, ISVNConflictResolver.Result.conflicted);
					conflictResolution = new ConflictResolution(descrip, editConflictsAction.getMergeConflictsCompareInput().getResolution());
					if (descrip.getConflictKind() == SVNConflictDescriptor.Kind.property)
						applyToAllPropertyResolution = null;
					else if (descrip.isBinary())
...
//		resolved = resolution != ISVNConflictResolver.Choice.postpone;
	...
	public int getResolvedConflictCount(int conflictKind) {
		if (textHandling == ISVNConflictResolver.Choice.postpone && binaryHandling == ISVNConflictResolver.Choice.postpone && treeConflictHandling == ISVNConflictResolver.Choice.postpone)
			return 0;
