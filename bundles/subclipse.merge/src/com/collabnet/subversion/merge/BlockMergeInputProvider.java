    ...
    if (wizardPages == null || initializePages) {
      standardPage =
          new MergeWizardStandardPage(
              "standardBlock",
              Messages.BlockMergeInputProvider_selectMergeSource,
              Activator.getDefault().getImageDescriptor(Activator.IMAGE_MERGE_WIZARD),
              Messages.BlockMergeInputProvider_specifyLocations,
              false,
              false); //$NON-NLS-1$
      revisionsPage =
          new MergeWizardRevisionsPage(
              "revisionsBlock",
              Messages.BlockMergeInputProvider_selectRevisions,
              Activator.getDefault().getImageDescriptor(Activator.IMAGE_MERGE_WIZARD),
              standardPage,
              Messages.BlockMergeInputProvider_specifyRevisions); // $NON-NLS-1$
  ...
  public boolean performMerge(
    ...
    try {
      ...
      MessageDialog.openError(
          ...
          Messages.BlockMergeInputProvider_merge,
          e.getMessage());
      return false;
    }
