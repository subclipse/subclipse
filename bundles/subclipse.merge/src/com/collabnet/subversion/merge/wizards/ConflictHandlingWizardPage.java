...
public class ConflictHandlingWizardPage extends WizardPage {
	...
	private final static String LAST_TREE_CHOICE = "ConflictHandlingDialog.lastTreeChoice";	 //$NON-NLS-1$

	public ConflictHandlingWizardPage(String pageName) {
		super(pageName, Messages.ConflictHandlingWizardPage_handleConflict, Activator.getDefault().getImageDescriptor(Activator.IMAGE_SVN));
	...
	public void createControl(Composite parent) {	
		...
		if (conflictResource == null) {
			...
			label.setText(Messages.ConflictHandlingWizardPage_resource + convertTempFileName(relativePath));
		}
		else label.setText(Messages.ConflictHandlingWizardPage_resource + conflictResource.getFullPath().makeRelative().toOSString());
		...
		if (conflictDescriptor.isTreeConflict()) {
			...
			if (conflictDescriptor.getTheirPath() != null) {
				chooseIncomingVersionButton = new Button(conflictGroup, SWT.RADIO);
				if (conflictDescriptor.getConflictKind() == SVNConflictDescriptor.Kind.property) {
					chooseIncomingVersionButton.setText(Messages.ConflictHandlingWizardPage_useIncomingProperty);		
				} else {
	...
	//			if (conflictDescriptor.getConflictKind() == SVNConflictDescriptor.Kind.property)
	//				chooseBaseVersionButton.setText("Resolve the conflict by using the &base version of the property.");		
	//			else
	//				chooseBaseVersionButton.setText("Resolve the conflict by using the &base version of the file.");		
	//		}
			if (!conflictDescriptor.isBinary()) {
			...
			if (showConflictEditorOption()) {
				...
				if (conflictDescriptor.getConflictKind() == SVNConflictDescriptor.Kind.property && conflictDescriptor.getMergedPath() == null)
					...
					conflictEditorButton.setText(Messages.ConflictHandlingWizardPage_graphicalEditor);	
			}
		}
		...
		if (conflictDescriptor.isTreeConflict())
			...
			applyToAllButton.setText(Messages.ConflictHandlingWizardPage_applyToAllText);
		...
		if (conflictDescriptor.isTreeConflict())
			...
			setMessage(Messages.ConflictHandlingWizardPage_cannotMergeProperty + conflictDescriptor.getPropertyName() + Messages.ConflictHandlingWizardPage_forResource + convertTempFileName(file.getName()) + "."); //$NON-NLS-1$
		else
			setMessage(Messages.ConflictHandlingWizardPage_cannotMerge + convertTempFileName(file.getName()) + "."); //$NON-NLS-1$ //$NON-NLS-1$
