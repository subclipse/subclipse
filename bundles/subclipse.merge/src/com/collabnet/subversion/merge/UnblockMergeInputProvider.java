    ...
    if (wizardPages == null || initializePages) {
      standardPage =
          new MergeWizardStandardPage(
              "standardUnblock",
              Messages.UnblockMergeInputProvider_selectMergeSource,
              Activator.getDefault().getImageDescriptor(Activator.IMAGE_MERGE_WIZARD),
              Messages.UnblockMergeInputProvider_specifyLocation,
              false,
              true); //$NON-NLS-1$
      revisionsPage =
          new MergeWizardUnblockRevisionsPage(
              "revisionsUnblock",
              Messages.UnblockMergeInputProvider_selectRevisions,
              Activator.getDefault().getImageDescriptor(Activator.IMAGE_MERGE_WIZARD),
  ...
  public boolean performMerge(
    ...
    try {
      ...
      MessageDialog.openError(
          ...
          Messages.UnblockMergeInputProvider_merge,
          e.getMessage());
      return false;
    }
