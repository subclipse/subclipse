...
public class MergeFileAssociationPreferencePage extends PreferencePage
  ...
  protected Control createContents(Composite parent) {
    ...
    fileTypesLabel.setText(
        Policy.bind("MergeFileAssociationPreferencePage.fileTypes")); // $NON-NLS-1$
    ...
    mergeProgramGroup.setText(
        Policy.bind("MergeFileAssociationPreferencePage.mergeProgramGroup")); // $NON-NLS-1$
    gridData = new GridData(GridData.FILL_HORIZONTAL);
    gridData.horizontalSpan = 2;
    ...
    builtInMergeRadioButton.setText(
        Policy.bind("DiffMergePreferencePage.builtInMerge")); // $NON-NLS-1$
    gridData = new GridData();
    gridData.horizontalSpan = 3;
    builtInMergeRadioButton.setLayoutData(gridData);

    externalMergeRadioButton = new Button(mergeProgramGroup, SWT.RADIO);
    externalMergeRadioButton.setText(
        Policy.bind("MergeFileAssociationPreferencePage.defaultExternal")); // $NON-NLS-1$
    gridData = new GridData();
    ...
    defaultParametersLabel.setText(
        Policy.bind("DiffMergePreferencePage.mergeProgramParameters")); // $NON-NLS-1$
    gridData = new GridData();
    ...
    defaultProgramLocationText.setText(
        store.getString(ISVNUIConstants.PREF_MERGE_PROGRAM_LOCATION));
    defaultProgramParametersText.setText(
    ...
    customMergeRadioButton.setText(
        Policy.bind(
            "MergeFileAssociationPreferencePage.customExternal")); //$NON-NLS-1$
    ...
    variablesButton.setText(
        Policy.bind("DiffMergePreferencePage.mergeProgramVariables")); // $NON-NLS-1$
    gridData = new GridData();
    gridData.horizontalAlignment = GridData.FILL;
            ...
            new SelectionAdapter() {
              public void widgetSelected(SelectionEvent e) {
                ...
                MergeFileAssociation mergeFileAssociation =
                    (MergeFileAssociation) selection.getFirstElement();
        ...
        new SelectionAdapter() {
          public void widgetSelected(SelectionEvent e) {
            ...
            if (e.getSource() == customProgramLocationCombo) {
              for (int i = 0; i < mergeFileAssociations.length; i++) {
                if (mergeFileAssociations[i].getType() == MergeFileAssociation.CUSTOM_EXTERNAL) {
                  if (mergeFileAssociations[i]
                      .getMergeProgram()
                      .equals(customProgramLocationCombo.getText())) {
            ...
            if (e.getSource() instanceof Button) {
              ...
              if (mergeFileAssociation != null) {
                ...
                else if (externalMergeRadioButton.getSelection())
                  type = MergeFileAssociation.DEFAULT_EXTERNAL;
                else if (customMergeRadioButton.getSelection())
                  type = MergeFileAssociation.CUSTOM_EXTERNAL;
  ...
  public boolean performOk() {
    ...
    for (int i = 0; i < mergeFileAssociations.length; i++) {
      Preferences prefs =
          MergeFileAssociation.getParentPreferences().node(mergeFileAssociations[i].getFileType());
      if (mergeFileAssociations[i].getMergeProgram() == null)
        prefs.put("mergeProgram", ""); // $NON-NLS-1$ //$NON-NLS-1$ //$NON-NLS-2$
      else prefs.put("mergeProgram", mergeFileAssociations[i].getMergeProgram()); // $NON-NLS-1$
      if (mergeFileAssociations[i].getParameters() == null)
        prefs.put("parameters", ""); // $NON-NLS-1$ //$NON-NLS-1$ //$NON-NLS-2$
  ...
  public boolean performCancel() {
    if (associationsUpdated) {
      if (MessageDialog.openQuestion(
          getShell(),
          Policy.bind("MergeFileAssociationPreferencePage.title"),
          Policy.bind(
              "MergeFileAssociationPreferencePage.confirmCancel"))) //$NON-NLS-1$ //$NON-NLS-1$
                                                                    // //$NON-NLS-2$
      performOk();
    }
  ...
  private void addFileType() {
    ...
    for (int i = 0; i < mergeFileAssociations.length; i++)
      associationsList.add(mergeFileAssociations[i]);
  ...
  static class MergeFileAssociationsLabelProvider extends LabelProvider
    ...
    public Image getColumnImage(Object element, int columnIndex) {
      ...
      ImageDescriptor id =
          PlatformUI.getWorkbench()
              .getEditorRegistry()
              .getImageDescriptor(mergeFileAssociation.getFileType());
      if (id == null)
        id =
            PlatformUI.getWorkbench()
                .getEditorRegistry()
                .getImageDescriptor("file.txt"); // $NON-NLS-1$
      if (id != null) {
