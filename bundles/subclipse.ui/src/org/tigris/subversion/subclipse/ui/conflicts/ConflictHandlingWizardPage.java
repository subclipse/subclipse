...
public class ConflictHandlingWizardPage extends WizardPage {
  ...
  public ConflictHandlingWizardPage(String pageName) {
    super(
        pageName,
        Messages.ConflictHandlingWizardPage_0,
  ...
  public void createControl(Composite parent) {
    ...
    if (conflictResource == null) {
      ...
      label.setText(
          Messages.ConflictHandlingWizardPage_2
    ...
    if (conflictDescriptor.getReason() == SVNConflictDescriptor.Reason.deleted
        || conflictDescriptor.getReason() == SVNConflictDescriptor.Reason.moved_away) {
      ...
      if (showConflictEditorOption()) {
        ...
        else conflictEditorButton.setText(Messages.ConflictHandlingWizardPage_11);
      }
    }
    ...
    if (conflictDescriptor.getReason() == SVNConflictDescriptor.Reason.deleted
      ...
      setMessage(
          "SVN cannot automatically merge property "
              + conflictDescriptor.getPropertyName()
              + Messages.ConflictHandlingWizardPage_15
              + convertTempFileName(file.getName())
              + "."); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
    else
      setMessage(
          "SVN cannot automatically merge file "
              + convertTempFileName(file.getName())
              + "."); //$NON-NLS-1$ //$NON-NLS-1$ //$NON-NLS-2$
