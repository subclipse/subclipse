...
public class PropertyCompareRemoteResourceNode
    implements IStructureComparator, ITypedElement, IStreamContentAccessor, IPropertyProvider {
  ...
  public PropertyCompareRemoteResourceNode(
  ...
  public void getProperties(boolean recursive) {
    ...
    try {
      ...
      properties =
          svnClient.getProperties(
              remoteResource.getUrl(), remoteResource.getRevision(), pegRevision, recursive);
    } catch (Exception e) {
  ...
  public String getLabel() {
    return Policy.bind("PropertyCompareRemoteResourceNode.0")
        + remoteResource.getName().replaceAll("%20", " ")
        + "@"
        + remoteResource.getRevision(); // $NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$ //$NON-NLS-4$
  }
      ...
      if (recursive && remoteResource.isContainer()) {
        try {
          ...
          for (ISVNRemoteResource childResource : childResources) {
            PropertyCompareRemoteResourceNode childNode =
                new PropertyCompareRemoteResourceNode(
                    childResource, pegRevision, recursive, properties);
