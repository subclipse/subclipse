...
public class SVNLocalBaseCompareInput extends CompareEditorInput implements ISaveableWorkbenchPart {
  ...
  public SVNLocalBaseCompareInput(
      ISVNLocalResource[] resources, SVNRevision revision, boolean readOnly)
            ...
            if (copiedFromUrl != null) {
              GetRemoteResourceCommand getRemoteResourceCommand =
                  new GetRemoteResourceCommand(
                      resources[i].getRepository(), copiedFromUrl, SVNRevision.HEAD);
    ...
    if (unaddedResources.size() > 0) {
      ...
      AddResourcesCommand addResourcesCommand =
          new AddResourcesCommand(workspaceRoot, unadded, IResource.DEPTH_INFINITE);
   ...
   */
  public SVNLocalBaseCompareInput(ISVNLocalResource[] resources, SVNRevision revision)
      throws SVNException, SVNClientException {
    ...
    if (localResourceNodes.length > 1) {
      ...
      title =
          Policy.bind(
              ...
              new Object[] {localResourceNodes[0].getName()}); // $NON-NLS-1$
      leftLabel =
          Policy.bind(
    ...
    if (differences instanceof DiffNode) {
      ...
      for (IDiffElement child : children) {
        if (child instanceof DiffNode) {
          ...
          if (childDiffNode.getLeft() == null
              && childDiffNode.getRight() instanceof ResourceEditionNode) {
