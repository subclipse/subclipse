...
/**
 * First wizard page for importing a project into a SVN repository. This page prompts the user to
 * select an existing repo or create a new one. If the user selected an existing repo, then
 * getLocation() will return it.
 */
public class RepositorySelectionPage extends SVNWizardPage {
  ...
  protected TableViewer createTable(Composite parent, int span) {
    Table table =
        new Table(
            parent, SWT.H_SCROLL | SWT.V_SCROLL | SWT.BORDER | SWT.SINGLE | SWT.FULL_SELECTION);
  ...
  public void createControl(Composite parent) {
    ...
    PlatformUI.getWorkbench()
        .getHelpSystem()
        .setHelp(composite, IHelpContextIds.SHARING_SELECT_REPOSITORY_PAGE);
    ...
    useNewRepo =
        createRadioButton(
            composite, Policy.bind("RepositorySelectionPage.useNew"), 1); // $NON-NLS-1$

    useExistingRepo =
        createRadioButton(
            composite, Policy.bind("RepositorySelectionPage.useExisting"), 1); // $NON-NLS-1$
          ...
          public void selectionChanged(SelectionChangedEvent event) {
            result =
                (ISVNRepositoryLocation)
                    ((IStructuredSelection) table.getSelection()).getFirstElement();
    ...
    IRunnableWithProgress runnable =
        new IRunnableWithProgress() {
          public void run(IProgressMonitor monitor)
            ...
            locations =
                SVNUIPlugin.getPlugin().getRepositoryManager().getKnownRepositoryLocations(monitor);
          }
        };
    try {
