...
public class ResolveSynchronizeOperation extends SVNSynchronizeOperation {
  ...
  public ResolveSynchronizeOperation(
                ...
                public void run() {
                  if (propertyConflicts && !textConflicts) {
                    ...
                    if (resources.length > 1)
                      message = Policy.bind("ResolveAction.confirmMultiple"); // $NON-NLS-1$
                    else
                      message =
                          Policy.bind(
                              "ResolveAction.confirm", resources[0].getName()); // $NON-NLS-1$
                    if (!MessageDialog.openConfirm(
                        getShell(),
                        Policy.bind("ResolveOperation.taskName"),
                        message)) { //$NON-NLS-1$
                      canceled = true;
                      return;
                    }
                    ...
                    SvnWizardMarkResolvedPage markResolvedPage =
                        new SvnWizardMarkResolvedPage(resources);
