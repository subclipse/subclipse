...
/**
 * A compare input for comparing local resource with its remote revision Perform textual check on: -
 * any local modification - revision numbers don't match
 */
public class SVNLocalCompareInput extends SaveableCompareEditorInput
    implements ISaveableWorkbenchPart {
  private Object fRoot;
  private final SVNRevision remoteRevision;
  private ISVNLocalResource resource;
  private ISVNRemoteResource
      remoteResource; // the remote resource to compare to or null if it does not exist
  ...
  /**
   * @throws SVNException creates a SVNLocalCompareInput, allows setting whether the current local
   *     resource is read only or not.
   */
  public SVNLocalCompareInput(ISVNLocalResource resource, SVNRevision revision, boolean readOnly)
      throws SVNException, SVNClientException {
        ...
        if (copiedFromUrl != null) {
          GetRemoteResourceCommand getRemoteResourceCommand =
              new GetRemoteResourceCommand(
                  resource.getRepository(), copiedFromUrl, SVNRevision.HEAD);
   ...
   *
   * @throws SVNException creates a SVNLocalCompareInput, defaultin to read/write.
   */
  public SVNLocalCompareInput(ISVNLocalResource resource, SVNRevision revision)
      throws SVNException, SVNClientException {
    this(resource, revision, false);
  }

  public SVNLocalCompareInput(ISVNLocalResource resource, ISVNRemoteResource remoteResource)
      throws SVNException {
    this(resource, remoteResource, null);
  }

  /** @throws SVNException creates a SVNCompareRevisionsInput */
  public SVNLocalCompareInput(
      ISVNLocalResource resource, ISVNRemoteResource remoteResource, SVNRevision pegRevision)
      throws SVNException {
    super(new CompareConfiguration(), SVNUIPlugin.getActivePage());
    this.resource = resource;
  ...
  private void initLabels() {
    ...
    String leftLabel =
        Policy.bind(
            "SVNCompareRevisionsInput.workspace", new Object[] {resourceName}); // $NON-NLS-1$
    ...
    String rightLabel =
        Policy.bind(
            "SVNCompareRevisionsInput.repository",
            new Object[] {remoteResourceName}); // $NON-NLS-1$
  ...
  protected ICompareInput prepareCompareInput(IProgressMonitor monitor)
    ...
    if (resource instanceof LocalFolder) {
      try {
        ...
        try {
          for (int i = 0; i < resources.length; i++) {
            ...
            SVNWorkspaceRoot workspaceRoot =
                new SVNWorkspaceRoot(resource.getResource().getProject());
            AddResourcesCommand command =
                new AddResourcesCommand(workspaceRoot, unaddedResources, IResource.DEPTH_INFINITE);
            command.run(monitor);

            diffSummary =
                client.diffSummarize(file, remoteFolder.getUrl(), remoteFolder.getRevision(), true);
            ...
            if (diffSummary != null && diffSummary.length > 0) {
              ...
              ITypedElement left =
                  new SVNLocalResourceSummaryNode(
                      resource, diffSummary, resource.getResource().getLocation().toString());
          ...
          if (resourceSummaryNodeList.size() == 0) {
            ...
            result[0] =
                new SummaryDifferencer().findDifferences(false, monitor, null, null, left, right);
            fRoot = result[0];
          }
        } finally {
          sub.done();
      ...
      if (SVNRevision.BASE.equals(remoteRevision)) {
        compareInput =
            (ICompareInput)
                new StatusAwareDifferencer()
                    .findDifferences(false, monitor, null, null, left, right);
      } else {
        compareInput =
            (ICompareInput)
                new RevisionAwareDifferencer(
                        (SVNLocalResourceNode) left, right, diffFile, pegRevision)
                    .findDifferences(false, monitor, null, null, left, right);
      }
      return compareInput;
    }
    ...
    for (int i = 0; i < diffSummary.length; i++) {
      ...
      while (file.getParentFile() != null) {
        ...
        if (!paths.contains(path)) {
          paths.add(path);
          SVNDiffSummary folder =
              new SVNDiffSummary(path, SVNDiffKind.NORMAL, false, SVNNodeKind.DIR.toInt());
          diffs.add(folder);
        }
      }
    }
                ...
                public boolean visit(IResource aResource) {
                  ...
                  try {
                    ...
                    if ((aResource.getType() == IResource.FOLDER)
                        && isSymLink(aResource)) // don't traverse into symlink folders
                    return false;
