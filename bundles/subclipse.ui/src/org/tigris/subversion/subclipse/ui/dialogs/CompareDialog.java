/**
 * ***************************************************************************** Copied from package
 * org.eclipse.compare.internal.CompareDialog, but features regarding writing the resource is taken
 * out.
 *
...
public class CompareDialog extends SubclipseTrayDialog {
    ...
    try {
      ...
      if (message != null) {
        MessageDialog.openError(
            shell, Policy.bind("CompareDialog.compareFailed"), message); // $NON-NLS-1$
        return false;
      }
      ...
      if (input.getCompareResult() == null) {
        MessageDialog.openInformation(
            shell,
            input.getTitle(),
            Policy.bind("CompareDialog.noDifferences")); // $NON-NLS-2$ //$NON-NLS-1$
        return false;
      }
      ...
      MessageDialog.openError(
          shell,
          Policy.bind("CompareDialog.compareFailed"),
  ...
  public int open() {
    if (compareResultOK(compareEditorInput)) return super.open();
