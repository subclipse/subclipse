...
public class ShowDifferencesAsUnifiedDiffOperationWC extends SVNOperation {
  ...
  public ShowDifferencesAsUnifiedDiffOperationWC(
      IWorkbenchPart part, File path, SVNUrl toUrl, SVNRevision toRevision, File file) {
    super(part);
    this.path = path;
    this.toUrl = toUrl;
  ...
  protected void execute(IProgressMonitor monitor) throws SVNException, InterruptedException {
    ...
    ISVNRepositoryLocation repository =
        SVNProviderPlugin.getPlugin().getRepository(toUrl.toString());
    if (repository != null) client = repository.getSVNClient();
    if (client == null) client = SVNProviderPlugin.getPlugin().getSVNClientManager().getSVNClient();
    try {
