...
public class SVNDecoratorPreferencesPage extends PreferencePage
  ...
  static {
    ...
    children.add(
        new PreviewFile(
            ...
            null,
            null)); //$NON-NLS-1$
    //		children.add(new PreviewFile("archive.zip", IResource.FILE, false, false, true, false, true,
    // false, false, false, false, false, false, null, null)); //$NON-NLS-1$
  ...
  static {
    dirty =
        new CachedImageDescriptor(
            TeamImages.getImageDescriptor(org.eclipse.team.ui.ISharedImages.IMG_DIRTY_OVR));
    checkedIn =
        new CachedImageDescriptor(
            TeamImages.getImageDescriptor(org.eclipse.team.ui.ISharedImages.IMG_CHECKEDIN_OVR));
    added =
        new CachedImageDescriptor(
            SVNUIPlugin.getPlugin().getImageDescriptor(ISVNUIConstants.IMG_ADDED));
    moved =
        new CachedImageDescriptor(
            SVNUIPlugin.getPlugin().getImageDescriptor(ISVNUIConstants.IMG_MOVED));
    newResource =
        new CachedImageDescriptor(
            SVNUIPlugin.getPlugin().getImageDescriptor(ISVNUIConstants.IMG_QUESTIONABLE));
    external =
        new CachedImageDescriptor(
            SVNUIPlugin.getPlugin().getImageDescriptor(ISVNUIConstants.IMG_EXTERNAL));
    locked =
        new CachedImageDescriptor(
            SVNUIPlugin.getPlugin().getImageDescriptor(ISVNUIConstants.IMG_LOCKED));
    needsLock =
        new CachedImageDescriptor(
            SVNUIPlugin.getPlugin().getImageDescriptor(ISVNUIConstants.IMG_NEEDSLOCK));
    conflicted =
        new CachedImageDescriptor(
            SVNUIPlugin.getPlugin().getImageDescriptor(ISVNUIConstants.IMG_CONFLICTED));
    deleted =
        new CachedImageDescriptor(
            SVNUIPlugin.getPlugin().getImageDescriptor(ISVNUIConstants.IMG_DELETED));
    switched =
        new CachedImageDescriptor(
            SVNUIPlugin.getPlugin().getImageDescriptor(ISVNUIConstants.IMG_SWITCHED));
  }
  ...
  private Control createGeneralDecoratorPage(Composite parent) {
    ...
    SWTUtils.createPreferenceLink(
        ...
        Policy.bind(
            "SVNDecoratorPreferencesPage.labelDecorationsLink")); //$NON-NLS-1$ //$NON-NLS-2$
    showDirty =
        createCheckBox(
            composite, Policy.bind("SVNDecoratorPreferencesPage.computeDeep")); // $NON-NLS-1$
    enableFontDecorators =
        createCheckBox(
            composite,
            Policy.bind("SVNDecoratorPreferencesPage.useFontDecorators")); // $NON-NLS-1$		
    SWTUtils.createPreferenceLink(
        (IWorkbenchPreferenceContainer) getContainer(),
        composite,
        "org.eclipse.ui.preferencePages.ColorsAndFonts",
        Policy.bind(
            "SVNDecoratorPreferencesPage.colorsAndFontsLink")); //$NON-NLS-1$ //$NON-NLS-2$
    return composite;
  }

  /**
   * creates the following controls (sample) File Format : [{added_flag}{dirty_flag}{name}
   * {revision} {date} {author}] [Add Variables] Example : [ ] supportedBindings is a map of {key :
   * description} (ex : {"name","name of the resource being decorated"})
   *
   * @returns the text control for the format and the text control for the example
   */
  protected TextPair createFormatEditorControl(
      Composite composite, String title, String buttonText, final Map supportedBindings) {
  ...
  protected Control createContents(Composite parent) {
    ...
    PlatformUI.getWorkbench()
        .getHelpSystem()
        .setHelp(getControl(), IHelpContextIds.DECORATORS_PREFERENCE_PAGE);
    PlatformUI.getWorkbench()
        .getThemeManager()
        .addPropertyChangeListener(fThemeListener = new ThemeListener(fPreview));
  ...
  private Control createTextDecoratorPage(Composite parent) {
    ...
    createLabel(
        fileTextGroup, Policy.bind("SVNDecoratorPreferencesPage.selectFormats"), 3); // $NON-NLS-1$

    TextPair format =
        createFormatEditorControl(
            fileTextGroup,
            Policy.bind("SVNDecoratorPreferencesPage.fileFormat"), // $NON-NLS-1$
            Policy.bind("SVNDecoratorPreferencesPage.addVariables"),
            getFileBindingDescriptions()); //$NON-NLS-1$ //$NON-NLS-2$
    ...
    format =
        createFormatEditorControl(
            ...
            Policy.bind("SVNDecoratorPreferencesPage.addVariables"),
            getFolderBindingDescriptions()); //$NON-NLS-1$ //$NON-NLS-2$
    ...
    format =
        createFormatEditorControl(
            ...
            Policy.bind("SVNDecoratorPreferencesPage.addVariables"),
            getProjectBindingDescriptions()); //$NON-NLS-1$ //$NON-NLS-2$
    ...
    dateFormatText.addModifyListener(
        new ModifyListener() {
          public void modifyText(ModifyEvent e) {
            updateExamples();
          }
  ...
  private Control createIconDecoratorPage(Composite parent) {
    ...
    imageShowHasRemote =
        createCheckBox(imageGroup, Policy.bind("Show_has_&remote_26")); // $NON-NLS-1$
    imageShowAdded = createCheckBox(imageGroup, Policy.bind("S&how_is_added_moved")); // $NON-NLS-1$
    imageShowNewResource =
        createCheckBox(
            imageGroup, Policy.bind("SVNDecoratorPreferencesPage.newResources")); // $NON-NLS-1$
    imageShowExternal =
        createCheckBox(
            imageGroup,
            Policy.bind("SVNDecoratorPreferencesPage.externalResources")); // $NON-NLS-1$
    imageShowReadOnly =
        createCheckBox(imageGroup, Policy.bind("SVNDecoratorPreferencesPage.1")); // $NON-NLS-1$
    return imageGroup;
  }
  ...
  private void initializeValues() {
    ...
    imageShowHasRemote.setSelection(
        store.getBoolean(ISVNUIConstants.PREF_SHOW_HASREMOTE_DECORATION));
    imageShowNewResource.setSelection(
        store.getBoolean(ISVNUIConstants.PREF_SHOW_NEWRESOURCE_DECORATION));
    imageShowExternal.setSelection(store.getBoolean(ISVNUIConstants.PREF_SHOW_EXTERNAL_DECORATION));
    imageShowReadOnly.setSelection(
        corePreferences.getBoolean(ISVNCoreConstants.PREF_SHOW_READ_ONLY));
  ...
  public boolean performOk() {
    ...
    store.setValue(
        ISVNUIConstants.PREF_SHOW_HASREMOTE_DECORATION, imageShowHasRemote.getSelection());
    store.setValue(
        ISVNUIConstants.PREF_SHOW_NEWRESOURCE_DECORATION, imageShowNewResource.getSelection());
    store.setValue(ISVNUIConstants.PREF_SHOW_EXTERNAL_DECORATION, imageShowExternal.getSelection());
    corePreferences.setValue(
        ISVNCoreConstants.PREF_SHOW_READ_ONLY, imageShowReadOnly.getSelection());
    ...
    SVNProviderPlugin.getPlugin()
        ...
        .setValue(ISVNCoreConstants.PREF_RECURSIVE_STATUS_UPDATE, showDirty.getSelection());
  ...
  protected void performDefaults() {
    ...
    imageShowDirty.setSelection(
        store.getDefaultBoolean(ISVNUIConstants.PREF_SHOW_DIRTY_DECORATION));
    imageShowAdded.setSelection(
        store.getDefaultBoolean(ISVNUIConstants.PREF_SHOW_ADDED_DECORATION));
    imageShowHasRemote.setSelection(
        store.getDefaultBoolean(ISVNUIConstants.PREF_SHOW_HASREMOTE_DECORATION));
    imageShowNewResource.setSelection(
        store.getDefaultBoolean(ISVNUIConstants.PREF_SHOW_NEWRESOURCE_DECORATION));
    imageShowExternal.setSelection(
        store.getDefaultBoolean(ISVNUIConstants.PREF_SHOW_EXTERNAL_DECORATION));
    imageShowReadOnly.setSelection(false);

    showDirty.setSelection(store.getDefaultBoolean(ISVNUIConstants.PREF_CALCULATE_DIRTY));
    enableFontDecorators.setSelection(
        store.getDefaultBoolean(ISVNUIConstants.PREF_USE_FONT_DECORATORS));
   ...
   * store our preferences separately from the desktop.
   *
  ...
  /**
   * Add another variable to the given target. The variable is inserted at current position A
   * ListSelectionDialog is shown and the choose the variables to add
   */
  private void addVariables(Text target, Map bindings) {
    ...
    IStructuredContentProvider contentsProvider =
    ...
    if (dialog.open() != ListSelectionDialog.OK) return;
  ...
  private Map getFolderBindingDescriptions() {
    ...
    bindings.put(
        SVNDecoratorConfiguration.RESOURCE_NAME,
        Policy.bind("SVNDecoratorPreferencesPage.nameResourceVariable")); // $NON-NLS-1$
    bindings.put(
        SVNDecoratorConfiguration.RESOURCE_REVISION,
        Policy.bind("SVNDecoratorPreferencesPage.revisionResourceVariable")); // $NON-NLS-1$
    bindings.put(
        SVNDecoratorConfiguration.DIRTY_FLAG,
        Policy.bind("SVNDecoratorPreferencesPage.flagDirtyVariable")); // $NON-NLS-1$
    bindings.put(
        SVNDecoratorConfiguration.ADDED_FLAG,
        Policy.bind("SVNDecoratorPreferencesPage.flagAddedVariable")); // $NON-NLS-1$
    bindings.put(
        SVNDecoratorConfiguration.EXTERNAL_FLAG,
        Policy.bind("SVNDecoratorPreferencesPage.flagExternalVariable")); // $NON-NLS-1$
    bindings.put(
        SVNDecoratorConfiguration.RESOURCE_AUTHOR,
        Policy.bind("SVNDecoratorPreferencesPage.authorVariable")); // $NON-NLS-1$
    bindings.put(
        SVNDecoratorConfiguration.RESOURCE_DATE,
        Policy.bind("SVNDecoratorPreferencesPage.dateVariable")); // $NON-NLS-1$
    return bindings;
  }
  ...
  private Map getProjectBindingDescriptions() {
    ...
    bindings.put(
        ...
        Policy.bind(
            "SVNDecoratorPreferencesPage.remoteLocationVariable")); //$NON-NLS-1$
    bindings.put(
        SVNDecoratorConfiguration.RESOURCE_URL_SHORT,
        Policy.bind(
            "SVNDecoratorPreferencesPage.remoteLocationVariableShort")); //$NON-NLS-1$
    bindings.put(
        SVNDecoratorConfiguration.RESOURCE_LABEL,
        Policy.bind(
            "SVNDecoratorPreferencesPage.remoteLocationLabel")); //$NON-NLS-1$
    bindings.put(
        SVNDecoratorConfiguration.RESOURCE_REVISION,
        Policy.bind("SVNDecoratorPreferencesPage.revisionResourceVariable")); // $NON-NLS-1$
    return bindings;
  ...
  public class Preview extends LabelProvider implements Observer, ITreeContentProvider {
    ...
    public Preview(Composite composite) {
      SWTUtils.createLabel(
          composite, Policy.bind("SVNDecoratorPreferencesPage.preview")); // $NON-NLS-1$
    ...
    public Color getBackground(Object element) {
      ...
      if (((PreviewFile) element).ignored) {
        ...
        return current
            .getColorRegistry()
            .get(SVNDecoratorConfiguration.OUTGOING_CHANGE_BACKGROUND_COLOR);
      }
      return null;
    }
    ...
    public Color getForeground(Object element) {
      ...
      if (((PreviewFile) element).ignored) {
        ...
        return current
            .getColorRegistry()
            .get(SVNDecoratorConfiguration.OUTGOING_CHANGE_FOREGROUND_COLOR);
      }
      return null;
    }
    ...
    public String getText(Object element) {
      ...
      bindings.put(
          SVNDecoratorConfiguration.RESOURCE_DATE, dateFormat.format(new Date())); // $NON-NLS-1$
      bindings.put(
          SVNDecoratorConfiguration.RESOURCE_URL,
          "http://localhost:8080/svn/repos/trunk/project1"); //$NON-NLS-1$
      ...
      if (previewFile.type == IResource.FILE)
        return SVNDecoratorConfiguration.decorate(
            previewFile.name, fileTextFormat.getText(), bindings); // $NON-NLS-1$
      else if (previewFile.type == IResource.FOLDER)
        return SVNDecoratorConfiguration.decorate(
            previewFile.name, folderTextFormat.getText(), bindings); // $NON-NLS-1$
      else if (previewFile.type == IResource.PROJECT)
        return SVNDecoratorConfiguration.decorate(
            previewFile.name, projectTextFormat.getText(), bindings); // $NON-NLS-1$
    ...
    public Image getImage(Object element) {
      ...
      if (overlay == null) return baseImage;
      try {
        return fImageCache.createImage(
            new OverlayIcon(
                baseImage,
                new ImageDescriptor[] {overlay},
                new int[] {OverlayIcon.BOTTOM_RIGHT},
