...
public class SvnWizardNewRepositoryPage extends SvnWizardDialogPage {
  ...
  public SvnWizardNewRepositoryPage() {
    super(
        "NewRepositoryDialog",
        Policy.bind("NewRepositoryDialog.title")); // $NON-NLS-1$ //$NON-NLS-2$		
  }
            ...
            try {
              ...
              if (provider.getRepositories().isKnownRepository(url, true)) {
                MessageDialog.openError(
                    getShell(),
                    Policy.bind("NewRepositoryDialog.title"),
                    Policy.bind("NewRepositoryDialog.alreadyExists")); // $NON-NLS-1$					
                success = false;
                return;
              }
              ...
              if (connectionButton.getSelection()) {
                ...
                ISVNRepositoryLocation repository =
                    provider.getRepositories().createRepository(properties);
