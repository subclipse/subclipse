...
/**
 * Action for checking in files to a subversion provider Prompts the user for a release comment, and
 * shows a selection list of added and modified resources, including unversioned resources. If
 * selected, unversioned resources will be added to version control, and committed.
 */
public class CommitAction extends WorkbenchWindowAction {
          ...
          new IRunnableWithProgress() {
            public void run(IProgressMonitor monitor) throws InvocationTargetException {
              try {
                ...
                IResource[] modified =
                    getChangeSetResources(getModifiedResources(resources, monitor));
  ...
  protected IResource[] getModifiedResources(
      IResource[] resources, IProgressMonitor iProgressMonitor) throws SVNException {
      ...
      for (int j = 0; j < statuses.length; j++) {
        if (SVNStatusUtils.isReadyForCommit(statuses[j]) || SVNStatusUtils.isMissing(statuses[j])) {
          ...
          if (currentResource != null
              && (descend == true || allSelections.contains(currentResource))) {

            SVNProviderPlugin.getPlugin()
                .getStatusCacheManager()
                .updateCache(currentResource, statuses[j]);
                  ...
                  if (currentResource instanceof IContainer)
                    ...
                    if (SVNStatusUtils.isTextConflicted(statuses[j])) {
                      IFile conflictNewFile =
                          (IFile) File2Resource.getResource(statuses[j].getConflictNew());
                      if (conflictNewFile != null) conflictFiles.add(conflictNewFile);
                      IFile conflictOldFile =
                          (IFile) File2Resource.getResource(statuses[j].getConflictOld());
                      if (conflictOldFile != null) conflictFiles.add(conflictOldFile);
                      IFile conflictWorkingFile =
                          (IFile) File2Resource.getResource(statuses[j].getConflictWorking());
   ...
   * @throws SVNException
   */
  protected boolean confirmCommit(
      IResource[] modifiedResources, ProjectProperties projectProperties) throws SVNException {
    IPreferenceStore preferenceStore = SVNUIPlugin.getPlugin().getPreferenceStore();
    boolean commitToTagsPathWithoutWarning =
        preferenceStore.getBoolean(ISVNUIConstants.PREF_COMMIT_TO_TAGS_PATH_WITHOUT_WARNING);
    ...
    switch (highestProblemSeverity) {
      case IMarker.SEVERITY_WARNING:
        String allowCommitsWithWarnings =
            preferenceStore.getString(ISVNUIConstants.PREF_ALLOW_COMMIT_WITH_WARNINGS);
        if (MessageDialogWithToggle.PROMPT.equals(allowCommitsWithWarnings)
          ...
          MessageDialogWithToggle warningDialog =
              MessageDialogWithToggle.openYesNoQuestion(
                  shell,
                  Policy.bind("CommitWizard.commitResources"),
                  Policy.bind("CommitWizard.warningMarkers"),
                  Policy.bind("CommitWizard.warningQuestion"),
                  false,
                  preferenceStore,
                  ISVNUIConstants
                      .PREF_ALLOW_COMMIT_WITH_WARNINGS); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
      ...
      case IMarker.SEVERITY_ERROR:
        String allowCommitsWithErrors =
            preferenceStore.getString(ISVNUIConstants.PREF_ALLOW_COMMIT_WITH_ERRORS);
        if (MessageDialogWithToggle.PROMPT.equals(allowCommitsWithErrors)
          ...
          MessageDialogWithToggle errorDialog =
              MessageDialogWithToggle.openYesNoQuestion(
                  shell,
                  Policy.bind("CommitWizard.commitResources"),
                  Policy.bind("CommitWizard.errorMarkers"),
                  Policy.bind("CommitWizard.errorQuestion"),
                  false,
                  preferenceStore,
                  ISVNUIConstants
                      .PREF_ALLOW_COMMIT_WITH_ERRORS); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
    ...
    SvnWizardCommitPage commitPage =
        new SvnWizardCommitPage(modifiedResources, url, projectProperties, statusMap, null, false);
  ...
  /**
   * @see
   *     org.tigris.subversion.subclipse.ui.actions.WorkspaceAction#isEnabledForUnmanagedResources()
   */
  ...
  protected boolean isEnabled() throws TeamException {
    ...
    for (int i = 0; i < resources.length; i++) {
      ...
      if (RepositoryProvider.getProvider(resource.getProject(), SVNProviderPlugin.getTypeId())
          == null) {
        return false;
      }
  ...
  private IResource[] getUnaddedResources(List resources, IProgressMonitor iProgressMonitor)
      throws SVNException {
  ...
  private boolean containsOne(ChangeSet set, IResource[] resourcesToCommit) {
    for (int j = 0; j < resourcesToCommit.length; j++) {
      ...
      if (set instanceof ActiveChangeSet) {
        ActiveChangeSet acs = (ActiveChangeSet) set;
        if (acs.getDiffTree().members(resource).length > 0) return true;
      }
    }
    return false;
  }
