...
public class SVNUIPreferenceInitializer extends AbstractPreferenceInitializer {
   ...
   * Note that this method shouldn't be called by the clients; it will be called automatically by
   * the preference initializer when the appropriate default preference node is accessed.
   */
  public void initializeDefaultPreferences() {
    IEclipsePreferences node = new DefaultScope().getNode(SVNUIPlugin.ID);
    node.put(
        ISVNUIConstants.PREF_CONSOLE_COMMAND_COLOR, StringConverter.asString(new RGB(0, 0, 0)));
    node.put(
        ISVNUIConstants.PREF_CONSOLE_MESSAGE_COLOR, StringConverter.asString(new RGB(0, 0, 255)));
    node.put(
        ISVNUIConstants.PREF_CONSOLE_ERROR_COLOR, StringConverter.asString(new RGB(255, 0, 0)));
    ...
    node.put(
        ISVNUIConstants.PREF_FILETEXT_DECORATION, SVNDecoratorConfiguration.DEFAULT_FILETEXTFORMAT);
    node.put(
        ISVNUIConstants.PREF_FOLDERTEXT_DECORATION,
        SVNDecoratorConfiguration.DEFAULT_FOLDERTEXTFORMAT);
    node.put(
        ISVNUIConstants.PREF_PROJECTTEXT_DECORATION,
        SVNDecoratorConfiguration.DEFAULT_PROJECTTEXTFORMAT);
    ...
    node.putInt(
        ISVNUIConstants.PREF_UPDATE_TO_HEAD_CONFLICT_HANDLING_TEXT_FILES,
        ISVNConflictResolver.Choice.postpone);
    node.putInt(
    ...
    node.putInt(
        ...
        ISVNConflictResolver.Choice.postpone);
    node.putInt(
        ISVNUIConstants.PREF_UPDATE_TO_HEAD_CONFLICT_HANDLING_TREE_CONFLICTS,
        ISVNConflictResolver.Choice.postpone);
