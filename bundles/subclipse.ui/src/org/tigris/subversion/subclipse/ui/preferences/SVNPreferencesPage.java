 ...
 * SVN Preference Page
 *
 * <p>Allows the configuration of SVN specific options.
 */
public class SVNPreferencesPage extends PreferencePage implements IWorkbenchPreferencePage {
  ...
  protected Control createContents(Composite parent) {
    ...
    showCompareRevisionInDialog =
        createCheckBox(
            composite, Policy.bind("SVNPreferencePage.showCompareMergeInSync")); // $NON-NLS-1$

    //		showUnadded = createCheckBox(composite, Policy.bind("SVNPreferencePage.showUnadded"));
    // //$NON-NLS-1$
    //
    //		selectUnadded = createCheckBox(composite, Policy.bind("SVNPreferencePage.selectUnadded"));
    // //$NON-NLS-1$

    removeOnReplace =
        createCheckBox(composite, Policy.bind("SVNPreferencePage.removeOnReplace")); // $NON-NLS-1$

    fetchChangePathOnDemand =
        createCheckBox(
            composite, Policy.bind("SVNPreferencePage.fetchChangePathOnDemand")); // $NON-NLS-1$

    showTagsInRemoteHistory =
        createCheckBox(composite, Policy.bind("SVNPreferencePage.showTags")); // $NON-NLS-1$

    showOutOfDateFolders =
        createCheckBox(
            composite, Policy.bind("SVNPreferencePage.showOutOfDateFolders")); // $NON-NLS-1$

    useJavaHLCommitHack =
        createCheckBox(
            composite, Policy.bind("SVNPreferencePage.useJavaHLCommitHack")); // $NON-NLS-1$

    shareNestedProjects =
        createCheckBox(
            composite, Policy.bind("SVNPreferencePage.shareNestedProjects")); // $NON-NLS-1$

    warnOnCommitToTagPath =
        createCheckBox(
            composite, Policy.bind("SVNPreferencePage.warnOnCommitToTagPath")); // $NON-NLS-1$

    ignoreHiddenChanges =
        createCheckBox(composite, Policy.bind("SVNPreferencesPage.0")); // $NON-NLS-1$
    ...
    resourcesWithWarningsPrompt =
        createRadio(groupWarnings, Policy.bind("prompt"), 1); // $NON-NLS-1$
    ...
    defaultConfigLocationRadio =
        createRadio(
            group, Policy.bind("SVNPreferencePage.useDefaultConfigLocation"), 3); // $NON-NLS-1$
    useDirectoryLocationRadio =
        createRadio(group, Policy.bind("SVNPreferencePage.useDirectoryConfig"), 1); // $NON-NLS-1$
    ...
    Listener configUpdateEnablements =
        new Listener() {
    ...
    browseConfigDirButton.addSelectionListener(
  ...
  private void initializeValues() {
    ...
    showCompareRevisionInDialog.setSelection(
        store.getBoolean(ISVNUIConstants.PREF_SHOW_COMPARE_REVISION_IN_DIALOG));

    fetchChangePathOnDemand.setSelection(
        store.getBoolean(ISVNUIConstants.PREF_FETCH_CHANGE_PATH_ON_DEMAND));

    showTagsInRemoteHistory.setSelection(
        store.getBoolean(ISVNUIConstants.PREF_SHOW_TAGS_IN_REMOTE));

    showOutOfDateFolders.setSelection(
        SVNProviderPlugin.getPlugin()
            .getPluginPreferences()
            .getBoolean(ISVNCoreConstants.PREF_SHOW_OUT_OF_DATE_FOLDERS));

    useJavaHLCommitHack.setSelection(store.getBoolean(ISVNUIConstants.PREF_USE_JAVAHL_COMMIT_HACK));

    shareNestedProjects.setSelection(
        SVNProviderPlugin.getPlugin()
            .getPluginPreferences()
            .getBoolean(ISVNCoreConstants.PREF_SHARE_NESTED_PROJECTS));

    warnOnCommitToTagPath.setSelection(
        !SVNUIPlugin.getPlugin()
            .getPluginPreferences()
            .getBoolean(ISVNUIConstants.PREF_COMMIT_TO_TAGS_PATH_WITHOUT_WARNING));

    ignoreHiddenChanges.setSelection(
        SVNProviderPlugin.getPlugin()
            .getPluginPreferences()
            .getBoolean(ISVNCoreConstants.PREF_IGNORE_HIDDEN_CHANGES));

    ignoreManagedDerivedResources.setSelection(
        SVNProviderPlugin.getPlugin()
            .getPluginPreferences()
    ...
    logEntriesToFetchText.setText(
        Integer.toString(store.getInt(ISVNUIConstants.PREF_LOG_ENTRIES_TO_FETCH)));

    quickDiffAnnotateYes.setSelection(
        MessageDialogWithToggle.ALWAYS.equals(
            store.getString(ISVNUIConstants.PREF_USE_QUICKDIFFANNOTATE)));
    quickDiffAnnotateNo.setSelection(
        MessageDialogWithToggle.NEVER.equals(
            store.getString(ISVNUIConstants.PREF_USE_QUICKDIFFANNOTATE)));
    quickDiffAnnotatePrompt.setSelection(
        MessageDialogWithToggle.PROMPT.equals(
            store.getString(ISVNUIConstants.PREF_USE_QUICKDIFFANNOTATE)));

    resourcesWithErrorsYes.setSelection(
        MessageDialogWithToggle.ALWAYS.equals(
            store.getString(ISVNUIConstants.PREF_ALLOW_COMMIT_WITH_ERRORS)));
    resourcesWithErrorsNo.setSelection(
        MessageDialogWithToggle.NEVER.equals(
            store.getString(ISVNUIConstants.PREF_ALLOW_COMMIT_WITH_ERRORS)));
    resourcesWithErrorsPrompt.setSelection(
        MessageDialogWithToggle.PROMPT.equals(
            store.getString(ISVNUIConstants.PREF_ALLOW_COMMIT_WITH_ERRORS)));

    resourcesWithWarningsYes.setSelection(
        MessageDialogWithToggle.ALWAYS.equals(
            store.getString(ISVNUIConstants.PREF_ALLOW_COMMIT_WITH_WARNINGS)));
    resourcesWithWarningsNo.setSelection(
        MessageDialogWithToggle.NEVER.equals(
            store.getString(ISVNUIConstants.PREF_ALLOW_COMMIT_WITH_WARNINGS)));
    resourcesWithWarningsPrompt.setSelection(
        MessageDialogWithToggle.PROMPT.equals(
            store.getString(ISVNUIConstants.PREF_ALLOW_COMMIT_WITH_WARNINGS)));
  ...
  public boolean performOk() {
    ...
    store.setValue(
        ...
        showCompareRevisionInDialog.getSelection());

    store.setValue(
        ISVNUIConstants.PREF_FETCH_CHANGE_PATH_ON_DEMAND, fetchChangePathOnDemand.getSelection());

    store.setValue(
        ISVNUIConstants.PREF_SHOW_TAGS_IN_REMOTE, showTagsInRemoteHistory.getSelection());

    if (SVNProviderPlugin.getPlugin()
            .getPluginPreferences()
            .getBoolean(ISVNCoreConstants.PREF_SHOW_OUT_OF_DATE_FOLDERS)
        != showOutOfDateFolders.getSelection()) {
      SVNProviderPlugin.getPlugin()
          .getPluginPreferences()
          .setValue(
              ISVNCoreConstants.PREF_SHOW_OUT_OF_DATE_FOLDERS, showOutOfDateFolders.getSelection());
      SVNUIPlugin.getPlugin()
          .getShowOutOfDateFoldersAction()
          .setChecked(showOutOfDateFolders.getSelection());
    }

    SVNProviderPlugin.getPlugin()
        .getPluginPreferences()
        .setValue(ISVNCoreConstants.PREF_SHARE_NESTED_PROJECTS, shareNestedProjects.getSelection());

    SVNUIPlugin.getPlugin()
        .getPluginPreferences()
        .setValue(
            ISVNUIConstants.PREF_COMMIT_TO_TAGS_PATH_WITHOUT_WARNING,
            !warnOnCommitToTagPath.getSelection());

    SVNProviderPlugin.getPlugin()
        .getPluginPreferences()
        .setValue(ISVNCoreConstants.PREF_IGNORE_HIDDEN_CHANGES, ignoreHiddenChanges.getSelection());
    ...
    //		store.setValue(ISVNUIConstants.PREF_SELECT_UNADDED_RESOURCES_ON_COMMIT,
    // selectUnadded.getSelection());

    // save remove unadded resources on replace
    store.setValue(
        ISVNUIConstants.PREF_REMOVE_UNADDED_RESOURCES_ON_REPLACE, removeOnReplace.getSelection());
    ...
    if (resourcesWithWarningsYes.getSelection()) {
      store.setValue(
          ISVNUIConstants.PREF_ALLOW_COMMIT_WITH_WARNINGS, MessageDialogWithToggle.ALWAYS);
    } else if (resourcesWithWarningsNo.getSelection()) {
      store.setValue(
          ISVNUIConstants.PREF_ALLOW_COMMIT_WITH_WARNINGS, MessageDialogWithToggle.NEVER);
    } else if (resourcesWithWarningsPrompt.getSelection()) {
      store.setValue(
          ISVNUIConstants.PREF_ALLOW_COMMIT_WITH_WARNINGS, MessageDialogWithToggle.PROMPT);
    }
  ...
  private abstract class ComboBox extends Field {
    ...
    public ComboBox(
      ...
      if (helpID != null) PlatformUI.getWorkbench().getHelpSystem().setHelp(fCombo, helpID);
    }
    ...
    public void initializeValue(IPreferenceStore store) {
      ...
      if (index >= 0 && index < fLabels.length) fCombo.select(index);
      else fCombo.select(0);
    }
