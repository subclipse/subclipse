...
public interface ISVNLocalResource extends ISVNResource, IAdaptable {
  ...
  /**
   * Answers the workspace synchronization information for this resource. This function does not
   * contact the server
   *
   * @return the synchronization information for this resource, or <code>null</code> if the resource
   *     does not have synchronization information available.
   * @throws SVNException
   */
  ...
  /**
   * Answers the revision number for this resource. The revision might not be stored in workspace
   * sychronization data, so a svn call might be required. This call however is not expected to
   * contact server, just fetch the revision from working copy metadata
   *
   * @return
   * @throws SVNException
   */
   ...
   *
   * <ul>
   *   <li>text status is added, deleted, replaced, modified, merged or conflicted.
  ...
  /**
   * Answer whether the resource could be ignored. Even if a resource is ignored, it can still be
   * added to a repository, at which time it should never be ignored by the SVN client.
   *
   * @throws SVNException
   */
